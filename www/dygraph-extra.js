...
Dygraph.Export.DEFAULT_ATTRS = {
    ...
    //Texts displayed below the chart's x-axis and to the left of the y-axis 
    axisLabelFont: "normal 12px sans-serif",
    ...
    legendFont: "normal 12px sans-serif",
    legendFontColor: "black",

    // Default position for vertical labels
    vLabelLeft: 8,

    legendHeight: 100,    // Height of the legend area
    legendMargin: 0,
    lineHeight: 17,
    maxlabelsWidth: 200,
	labelTopMargin: 35,
...
Dygraph.Export.asCanvas = function (dygraph, userOptions) {
    ...
    var numLegendLines = 0;
    for (var i = 0; i < labelVisibility.length; i++) {
        if (labelVisibility[i]) {
            numLegendLines++;
        }
    }
    options.legendHeight = numLegendLines * options.lineHeight
...
Dygraph.Export.putVerticalLabelY1 = function (ctx, divLabel, options, font, color, textAlign) {
    ...
    if (textAlign == "center") {
        ...
        top = Math.ceil((ctx.canvas.height - textDim.width) / 2 + textDim.width / 2);
    }
...
Dygraph.Export.drawLegend = function (canvas, dygraph, options) {
    ...
    for (i = 0; i < labels.length; i++) {
        if (labelVisibility[i]) {
            ...
            labelsY = labelsY + options.lineHeight;
        }
    }
};
