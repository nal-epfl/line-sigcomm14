...
def nicePlotInternal(plot):
  ...
  for item in plot['data']:
    if item['type'] == 'line':
      ...
      if 'whiskerColor' in item:
        plt.setp(p['whiskers'], color=colorsys.hsv_to_rgb(item['whiskerColor'][0], item['whiskerColor'][1], item['whiskerColor'][2]))
      else:
  ...
  plt.figtext(0.5, (1.0 + 0.5 * plot['xLabel'].count('\n')) * float(fontSizeAxisLabel * plot['fontScale']) / plt.gcf().dpi / plt.gcf().get_figheight(), plot['xLabel'], ha='center', va='center', fontsize=fontSizeAxisLabel * plot['fontScale'])
  plt.figtext((0.75 + 0.5 * plot['yLabel'].count('\n')) * float(fontSizeAxisLabel * plot['fontScale'] * fontSizeLineSize) / plt.gcf().dpi / plt.gcf().get_figwidth(), 0.5, plot['yLabel'], ha='center', va='center', rotation='vertical', fontsize=fontSizeAxisLabel * plot['fontScale'])
  ...
  if 'xTicks' in plot:
    if 'xTickLabels' in plot:
      plt.xticks(plot['xTicks'], plot['xTickLabels'], rotation=xTickRotation)
    else:
      plt.xticks(plot['xTicks'], rotation=xTickRotation)
  else:
    if 'xTickLabels' in plot:
      plt.gca().set_xticklabels(plot['xTickLabels'], rotation=xTickRotation)
  ...
  plt.gca().set_position([box.x0, box.y0, 1.0 - box.x0 - 0.25 / plt.gcf().get_figwidth(), 1.0 - box.y0 - 0.25 / plt.gcf().get_figheight()])
